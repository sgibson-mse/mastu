...
for ix in range(nx):
    for iy in range(ny):
        ...
        if (SXD_POLYGON_MASK(p1.x, p1.y) or SXD_POLYGON_MASK(p2.x, p2.y)
            or SXD_POLYGON_MASK(p3.x, p3.y) or SXD_POLYGON_MASK(p4.x, p4.y)):
...
file_name = "sxdl_rectilinear_grid.pickle"
